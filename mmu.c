...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
    ...
    */
    node_t *prev = NULL, *current = freelist->head, *best = NULL, *bestPrev = NULL;
    int bestSize = INT_MAX;

    // loop for each node in the freelist
    while (current != NULL) {
        int blockSize = current->blk->end - current->blk->start + 1;  //set blocksize for memory allocation
        if (blockSize >= blocksize) {
            if (policy == 1) { // policy one FIFO)
                break;
            } else if (policy == 2 && blockSize < bestSize) { // policy two bestfit
                best = current;  //set bestfit to the current block
                bestPrev = prev;  //set bestfit prev to the prev block
                bestSize = blockSize; //set the blocksize for the bets fit to the current block
            } else if (policy == 3 && blockSize > bestSize) { // policy three worst fit
                best = current;
                bestPrev = prev;
                bestSize = blockSize;
            }
...
/*
    ...
    block_t *allocBlock = current->blk;
    int ogBlockEnd = allocBlock->end;
  //task 3  
    allocBlock->pid = pid;
  //TASK 4
    allocBlock->end = allocBlock->start + blocksize - 1;
  
  // TASK NUMBER 5
    list_add_ascending_by_address(alloclist, allocBlock);
    if (blockSize > blocksize) {
      ///--------------------number 6 TASK A
  ...
  //      block_t *fragment = malloc(sizeof(block_t));
        ...
        fragment->pid = 0;
      ///--------------------number 6 TASK C
        fragment->start = allocBlock->end + 1;
      ///--------------------number 6 TASK D
        fragment->end = ogBlockEnd;
      ///--------------------number 6 TASK E
        if (policy == 1) {
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
    ...
    */
    ...
    // ---------------TASK 1
    while (current != NULL) {
        if (current->blk->pid == pid) {found = true; break;}
        prev = current;
        current = current->next;
    }
    //-----------TASK 2
    if (found == false) {printf("Error: Can't locate Memory Used by PID: %d\n", pid);return;}
    // Remove from allocated list
    block_t *deallocBlock = current->blk;
    if (prev == NULL) {
    ...
    deallocBlock->pid = 0;
    if (policy == 1) {
        list_add_to_back(freelist, deallocBlock);
    } else if (policy == 2) {
        list_add_ascending_by_blocksize(freelist, deallocBlock);
    } else { // policy == 3
        list_add_descending_by_blocksize(freelist, deallocBlock);
    }
}
